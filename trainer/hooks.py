"""Implementation of several hooks (routines) used in a Trainer class.
...
def train_hook_default(model,
    ...
    """
    model = model.train()
    iterator = iterator_type(loader,
                             disable=not stage_progress,
                             dynamic_ncols=True)
    ...
    for i, sample in enumerate(iterator):
        ...
        status = "{0}[Train][{1}] Loss_avg: {2:.03f}, Loss: {3:.03f}, LR: {4:.5}".format(
            prefix, i, loss_avg.avg, loss_avg.val,
...
def test_hook_default(model,
    ...
    for i, sample in enumerate(iterator):
        ...
        status = "{0}[Test][{1}] Loss_avg: {2:.05f}".format(
            prefix, i, loss_avg.avg)
        if get_key_metric is not None:
            status = status + ", Metric_avg: {0:.5}".format(
                get_key_metric(metric_fn.get_metric_value()))
        iterator.set_description(status)
...
def end_epoch_hook_classification(iterator, epoch, output_train, output_test):
    ...
    if hasattr(iterator, "set_description"):
        iterator.set_description(
            "epoch: {0}, test_top1: {1:.5}, train_loss: {2:.05f}, test_loss: {3:.05f}"
            .format(epoch, output_test["metric"]["top1"], output_train["loss"],
                    output_test["loss"]))
